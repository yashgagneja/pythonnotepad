...
main_application.geometry("640x400")
main_application.title("NOTPAD ProMAX")
#main_menu
main_menu=tk.Menu()
...
font_icon=tk.PhotoImage(file='icon/font_color.png')
...
screenshot_button=ttk.Button(tool_bar,text="Shot")
screenshot_button.grid(row=0,column=9,padx=5)
#End
...
def set_font_family(main_application):
    global current_font_family 
    current_font_family=font_family.get()
    text_editor.configure(font=(current_font_family,current_font_size))

def set_font_size(main_application):
    global current_font_size 
    current_font_size=size.get()
    text_editor.configure(font=(current_font_family,current_font_size))

font_box.bind("<<ComboboxSelected>>",set_font_family)
size_box.bind("<<ComboboxSelected>>",set_font_size)
...
screenshot_button.configure(command=screenshot)    
...
def change_status(event=None):
    if text_editor.edit_modified():
        ...
        character=len(text_editor.get(1.0,'end'))
...
def save_file(event=None):
    ...
    try:
        if url:
            ...
            url=filedialog.asksaveasfile(mode='w',default='.text',filetypes=(('Text file','*.txt'),('All files','*.*')))
...
def exit(event=None):
    ...
    try:
        if text_change:
            mbox=messagebox.askyesnocancel('Warning','Do u want to savve file')
            if mbox is True:
                if url:
...
def find_replace():
    ...
    find_dilog=tk.Toplevel()
    find_dilog.geometry('400x300+500+200')
    find_dilog.title('Find and Replace')
    find_dilog.resizable(0,0)
    find_frame=ttk.LabelFrame(find_dilog,text='Find/Replace')
    find_frame.pack(pady=20)
    text_find_frame=ttk.Label(find_frame,text='Find')
    text_find_frame.grid(row=0,column=0,padx=20,pady=20)
    text_replace_frame=ttk.Label(find_frame,text='Replace')
    text_replace_frame.grid(row=1,column=0,padx=20,pady=20)
    find_input=ttk.Entry(find_frame,width=30)
    find_input.grid(row=0,column=1,padx=20,pady=20)
    relace_input=ttk.Entry(find_frame,width=30)
    relace_input.grid(row=1,column=1,padx=20,pady=20)
    find_btn=ttk.Button(find_frame,text='Find',command=find)
    find_btn.grid(row=2,column=0,padx=20,pady=20)
    replace_btn=ttk.Button(find_frame,text='Replace',command=replace)
    replace_btn.grid(row=2,column=1,padx=20,pady=20)
...
edit.add_command(label='Clear' ,image=clear_icon, compound=tk.LEFT,command=lambda:text_editor.delete(1.0,tk.END))
edit.add_command(label='Find' ,image=find_icon, compound=tk.LEFT,command=find_replace)
...
view.add_checkbutton(label='toolbar', image=tool_bar_icon,onvalue=True,offvalue=False,variable=show_toolbar, compound=tk.LEFT,command=hide_toolbar)
view.add_checkbutton(label='statusbar', image=status_bar_icon,onvalue=True,offvalue=False,variable=show_statusbar, compound=tk.LEFT,command=hide_statusbar)
